<template>
  <div class="page">
    <div class="border-r">
      ...
      <Tree :value="root" selectionMode="single" @contextmenu.prevent="showMenu()" :expandedKeys="expandedKeys"
            v-model:selectionKeys="selectedKey" @node-select="selectTreeNode"></Tree>
    </div>
    <div class="content">
      ...
      <div class="p-p-3" style="height:calc(100% - 50px)">
        ...
        <div class="table-head">
          <div style="width:20%">
            <Button :class="{'p-disabled':table.length===0}" @click="deleteDialog(null)">
              {{
                this.state == 0 ? "清空安装成功的日志" : (this.state == 1 ? "清空安装失败的日志" : "清空安装中的日志")
              }}
            </Button>
          </div>
          <div class="queryForm">
        ...
        <div class="dataTable" style="overflow:hidden;height:calc(100% - 80px)">
          <DataTable :scrollHeight="scrollHeight" frozenWidth="10rem" :rowHover="true" :value="table"
                     :scrollable="true">
            <template #empty>
            ...
            <Column style="text-align:center;min-width: 12rem;" field="hostname" header="主机名">
              <template #body="row">
                <p v-tooltip.top="row.data.hostname"
            ...
            <Column style="text-align:center;min-width: 6rem;" field="domain" header="所属域">
              <template #body="row">
                <p v-tooltip.top="row.data.domain"
                   style="width: 100%;text-overflow: ellipsis;white-space: nowrap;overflow: hidden">{{
                    row.data.domain
                  }}</p>
              </template>
            ...
            <Column style="text-align:center;min-width: 6rem;" field="domain" header="系统名称">
              <template #body="row">
                <p v-tooltip.top="row.data.sysName"
                   style="width: 100%;text-overflow: ellipsis;white-space: nowrap;overflow: hidden">{{
                    row.data.sysName
                  }}</p>
              </template>
            ...
            <Column style="text-align:center;min-width: 8rem;" field="clientVersion" header="客户端版本">
              <template #body="row">
                <p v-tooltip.top="row.data.clientVersion"
                   style="width: 100%;text-overflow: ellipsis;white-space: nowrap;overflow: hidden">{{
            ...
            <Column style="text-align:center;min-width: 10rem;" field="sysVersion" header="系统版本">
              <template #body="row">
                <p v-tooltip.top="row.data.sysVersion"
                   style="width: 100%;text-overflow: ellipsis;white-space: nowrap;overflow: hidden">{{
                    row.data.sysVersion
                  }}</p>
              </template>
            ...
            <column style="text-align:center;min-width: 10rem;" header="系统兼容性">
              <template #body="row">
                <p v-tooltip.top="row.data.sysCmpt"
                   style="width: 100%;text-overflow: ellipsis;white-space: nowrap;overflow: hidden">
                  {{ row.data.sysCmpt }}</p>
              </template>
            </column>
            ...
            <Column style="text-align:center;min-width: 15rem;" field="beginTime" header="开始安装时间">
              <template #body="row">
                <span
                    v-tooltip.top="row.data.beginTime ? new Date(row.data.beginTime).toLocaleString('chinese',{hour12:false}).replace(/\//g, '-') : ''">{{
            ...
            <Column style="text-align:center;min-width: 15rem;" field="endTime" header="结束安装时间">
              <template #body="row">
                <span
                    v-tooltip.top="row.data.endTime ? new Date(row.data.endTime,).toLocaleString('chinese',{hour12:false}).replace(/\//g, '-') : '' ">{{
            ...
            <Column :frozen="true" align-frozen="right" style="min-width: 15rem;" header="操作">
              <template #body="row">
                <Button class="p-button-sm p-button-link" @click="infoDialog(row.data)">查看详情
                </Button>
                ...
                <Button class="p-button-sm p-button-link" style="color: red"
                        @click="deleteDialog(row.data)">删除
                </Button>
              </template>
            </Column>
          </DataTable>
...
export default {
  ...
  methods: {
    ...
    deleteLog() {
      if (this.deleteData) {
        this.$http
            .delete("/clientLeaveLog/" + this.deleteData.id)
            .then(() => {
              this.$toast.add({
                severity: "info",
                summary: "删除成功！ ",
