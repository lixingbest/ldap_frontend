<template>
  <Sidebar style="width: 800px" @hide="closeDialog" position="right" :showCloseIcon="false" v-model:visible="show">
    ...
    <div class="table-title">
      <div>
        <Button class="p-mr-3" @click="selectList.length>0?AddDialog():message('请先选择人员！')">添加到组</Button>
      </div>
      <div class="from">
        <div class="form-item p-mr-3">
          <label>登录名(UID) ：</label>
          <InputText v-model="query.uid" class="inout-t"></InputText>
        </div>
        <div class="form-item p-mr-3">
          <label>手机号码 ：</label>
          <InputText v-model="query.phone" class="inout-t"></InputText>
        </div>
      ...
      <Column style="min-width: 150px" field="title" header="职称">
        <template #body="a">
          <span class="center" v-for="(item ,index) in a.data.title" :key="item.index">
            {{$global.Util.isJSON(item)? JSON.parse(item).title:'' }}<span v-if="index!==a.data.title.length-1">,</span>
          </span>
        </template>
      </Column>
    ...
    <Dialog v-model:visible="deleteDisplay" @hide="closeAdd" header="添加用户" style="min-width: 250px">
