<template>
  <div>
    <div class="table-heard">
      <Button @click="addOrUpdateUser(null)">添加计划用户</Button>
    ...
    <data-table :value="table" frozenWidth="15rem" :rowHover="true" :scrollable="true" scroll-height="400px" paginator :rows="10" :rowsPerPageOptions="[10, 20, 50,100]"
      ...
      <Column field="uid" header="登录名">
        <template #body="row">
          {{ row.data.uid ? row.data.uid[0] : null }}
        </template>
      </Column>
      <Column field="cn" header="姓名">
        <template #body="row">
          {{ row.data.cn ? row.data.cn[0] : null }}
        </template>
      </Column>
      <Column field="employeenumber" header="工号">
        <template #body="row">
          {{ row.data.employeenumber ? row.data.employeenumber[0] : null }}
        </template>
      </Column>
      <Column field="employeenumber" header="状态">
        <template #body="a">
          <div class="center">
      ...
      <Column field="telephonenumber" header="固定电话">
        <template #body="row">
          {{ row.data.telephonenumber ? row.data.telephonenumber[0] : null }}
        </template>
      </Column>
      <Column field="mobile" header="手机号码">
        <template #body="row">
      ...
      <Column :frozen="true" alignFrozen="right" style="min-width: 15rem" header="操作">
        <template #body="row">
          <Button class="p-button-link p-button-sm" @click="showDialog(row.data,2)">激活</Button>
  ...
  <Dialog v-model:visible="display" :modal="true" @hide="closeTemplate()" :header="tableItem&&tableItem._type===1?'删除用户':'激活用户'">
    <div class="confirmation-content">
      <i class="pi pi-exclamation-triangle p-mr-3" style="font-size: 2rem" />
      <span v-if="tableItem&&tableItem._type===1"> 您确认要移除域用户 "{{ tableItem.uid[0] }}" 吗？</span>
      <span v-if="tableItem&&tableItem._type===2"> 您确认要激活用户 "{{ tableItem.uid[0] }}" 吗？</span>
    </div>
    <template #footer>
      <Button label="是" icon="pi pi-check" @click="tableItem&&tableItem._type===1?deleteUser():activeUser()" class="p-button-link" />
...
export default {
  ...
  methods: {
    ...
    addOrUpdateUser(data) {
      ...
      this.$nextTick(() => {
        this.$refs.addOrUpdate.init(data, this.domainId, domainId, 2);
      });
    },
    closeTemplate() {
