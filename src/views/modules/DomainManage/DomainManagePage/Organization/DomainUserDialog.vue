<template>
  <Sidebar style="width: 1000px" @hide="closeDialog" position="right" :showCloseIcon="false" v-model:visible="show">
    <h5>"{{ parentData.name }}"中的域用户</h5>
    ...
    <DataTable style="margin-top: 10px;width: 100%" :loading="loading" frozenWidth="100px" :rowHover="true" :value="list" :scrollable="true" scroll-height="400px" paginator :rows="10"
      ...
      <Column style="min-width: 150px" field="cn" header="姓名">
        <template #body="a">
      ...
      <Column field="dn" style="min-width: 350px;" header="可分辨名称(DN)">
        <template #body="a">
          <span class="text" v-tooltip.top="a.data.dn">{{ a.data.dn }}</span>
        </template>
      </Column>
      <Column :frozen="true" alignFrozen="right" style="min-width: 100px;justify-content: center" header="操作">
        <template #body="a">
          <Button class="p-button-link " style="color:red" @click="delDialog(a.data)">移除</Button>
        </template>
      </Column>
    </DataTable>

    <Dialog v-model:visible="delDisplay" @hide="closeDel" header="移除用户" style="min-width: 250px">
      <div class="confirmation-content">
        <i class="pi pi-exclamation-triangle p-mr-3" style="font-size: 2rem" />
        <span>
          您确定移除{{ delItem.uid[0] }}吗?
        </span>
      </div>
      <template #footer>
        <Button label="是" icon="pi pi-check" @click="submitDel" class="p-button-link" />
        <Button label="否" icon="pi pi-times" @click="deleteDisplay=false" class="p-button-secondary p-button-link" />
      </template>
    </Dialog>
    <CreateUser v-if="createDisplay" ref="createUser" v-on:close="closeAddUser"></CreateUser>
  </Sidebar>
</template>
...
export default {
  ...
  methods: {
    ...
    adduser() {
      ...
      let domainId = this.parentData.id + "-" + "1";
      this.$nextTick(() => {
        this.$refs.createUser.init(null, domainId);
      });
    },
    delDialog(item) {
